...
scheduler(void)
{
  ...
  c->proc = 0;
  for(;;){
    int low_nice_value = 21;
    int boost_nice_value = 21;
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
     if(p->state == RUNNABLE && p->nice_value<low_nice_value)
     {
      low_nice_value = p->nice_value;
     }
    }
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->nice_value == low_nice_value && p->state == RUNNABLE)
      // if(p->state == RUNNABLE)
      {
    ...
    for(struct proc * p_look = ptable.proc; p_look < &ptable.proc[NPROC]; p_look++)
    {
      for(int i = 0; i < p_look->num_lock; i++)
      {
        if(p_look->chan==p->lock_arr[i]&&p_look->nice_value<boost_nice_value){
          boost_nice_value = p_look->nice_value;
          p->nice_value = boost_nice_value;
        }
      } 
    }
